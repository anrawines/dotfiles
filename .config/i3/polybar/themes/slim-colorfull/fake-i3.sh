...
for ws in "${workspaces[@]}"; do
    ...
    if [ "$ws" == "$focused" ]; then
        ...
        output+="$action%{T7}%{F#000000}%{B#976ED3} $ws %{B-}%{F-}%{T-}$end_action"
        ...
        output+="$action%{T8}%{F#ffffff}%{B#4A3396} $ws %{B-}%{F-}%{T-}$end_action"
    else
        # KOSONG: Hanya teks redup, tanpa background
        output+="$action%{F#8D6BC9}%{B#0E1430} $ws %{B-}%{F-}$end_action"
    fi
